...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    ...
    assert sigmoid(a) <= sigmoid(
        a + EPS
    )  # Should be < for strictly increasing but computer's approximation doesn't see it that way :)
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    p ⇒ q ⇔ ¬p ∨ q
    """
    assert (lt(a, b) == 0.0 or lt(b, c) == 0.0) or lt(a, c) == 1.0
...
def test_other() -> None:
    ...
    """
    pytest.raises(ValueError, inv, 0)
    pytest.raises(ValueError, inv_back, 0, 0)
    pytest.raises(ValueError, log, -2)
    pytest.raises(ValueError, log_back, 0, 0)
